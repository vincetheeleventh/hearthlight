...
postProductionJson = async function(url, payload = {}) {
...
reviewAssetForShot = function(shot) {
  const newerImage = shot.newerPendingAsset?.kind === "image" ? shot.newerPendingAsset : null;
  return newerImage
    || (shot.assetHistory || []).find((asset) => asset.kind === "image" && ["style-composition", "likeness"].includes(asset.stage) && !asset.stale)
    || (shot.assetHistory || []).find((asset) => asset.kind === "image" && asset.stage !== "storyboard" && !asset.stale)
...
refreshProductionRoute = async function() {
  productionState.list = null;
  productionState.production = null;
  productionState.shot = null;
  await restoreStudioRoute();
...
function productionQuickReview(production, shot, notice) {
  const controls = node("details", "shot-quick-review");
  const target = reviewAssetForShot(shot);
  const summary = node("summary", "shot-quick-review-summary");
  summary.append(
    node("strong", "", "Review image"),
    node("span", "", target ? (assetApproved(target) ? "Passed" : target.reviewState === "revision-requested" ? "Changes flagged" : "Pending") : "No generated image"),
  );
  const body = node("div", "shot-quick-review-body");
  const note = node("textarea", "shot-feedback-input");
  note.rows = 2;
  note.placeholder = target ? "What needs to change?" : "No generated image to review yet";
  note.disabled = !target;
  ...
  body.append(note, actions, productionPromptEditor(production, shot, notice));
  controls.append(summary, body);
  return controls;
}
...
function openShotListControl(production, notice) {
  ...
  button.type = "button";
  button.disabled = !production.shotListDocument;
  button.title = production.shotListDocument?.path || "No shot-list spreadsheet registered";
  button.addEventListener("click", async () => {
    try {
      const result = await productionButton(button, () => postProductionJson(`/api/productions/${encodeURIComponent(production.slug)}/open-shot-list`, {}), "Opening...");
      showProductionNotice(notice, `Opened ${result.name}.`, "success");
    } catch (error) {
      showProductionNotice(notice, error.message, "error");
    }
  });
  control.append(button, node("span", "", production.shotListDocument?.name || "No spreadsheet registered"));
  return control;
}

function productionRequirementsPanel(production) {
  const unresolved = (production.requirements || []).filter((item) => !["approved", "not applicable"].includes(item.status));
  const current = unresolved.filter((item) => item.urgency === "now");
  const laterItems = unresolved.filter((item) => item.urgency === "later");
  const panel = node("details", "production-requirements");
  const summary = node("summary", "production-requirements-summary");
  const summaryCopy = node("div");
  summaryCopy.append(
    node("strong", "", "Requirements & assets"),
    node("span", "", "Character sheets, environments, source documents, and later-stage inputs"),
  );
  summary.append(summaryCopy, badge(`${unresolved.length} need attention`, unresolved.length ? "warning" : ""));
  const status = node("section", "production-status-grid");
  const now = node("div", "requirement-group");
  now.append(node("h3", "", `Current ? ${current.length}`));
  if (current.length) current.forEach((item) => now.append(requirementCard(item)));
  else now.append(node("p", "production-clear", "No current dependency gaps."));
  const later = node("div", "requirement-group requirement-later");
  later.append(node("h3", "", `Later ? ${laterItems.length}`));
  if (laterItems.length) laterItems.forEach((item) => later.append(requirementCard(item)));
  else later.append(node("p", "production-clear", "No later-stage gaps."));
  status.append(now, later);
  panel.append(summary, status);
  const stateKey = `hearthlight:requirements:${production.slug}`;
  try { panel.open = window.localStorage.getItem(stateKey) === "open"; } catch (_error) { panel.open = false; }
  panel.addEventListener("toggle", () => {
    try { window.localStorage.setItem(stateKey, panel.open ? "open" : "closed"); } catch (_error) { /* Browser storage unavailable. */ }
  });
  return panel;
}
renderProductionOverview = function(production) {
  ...
  const action = node("aside", "production-next-action");
  action.append(node("span", "eyebrow", "Next action"), node("strong", "", production.nextAction?.label || "Review project state"));
  const headingActions = node("div", "production-heading-actions");
  headingActions.append(openShotListControl(production, notice), action);
...
renderShotDetail = function(data) {
  ...
  dependencies.append(node("p", `dependency-explainer ${dependencyIssues.length ? "" : "is-clear"}`.trim(),
    dependencyIssues.length
      ...
      : "All declared shot dependencies are present and approved."));
  const dependencyGrid = node("div");
  for (const requirement of shot.requirements || []) dependencyGrid.append(requirementCard(requirement));
  dependencies.append(dependencyGrid);
  page.append(dependencies);
