...
| Input | Where | Owner |
|---|---|---|
| Approved storyboard (Gate 4) | `projects/{slug}/05-storyboard/storyboard.md` | `hearthlight-storyboard` |
...
3. **Gate 3 — Vince approves the still.** Stage A (spec compliance: verbatim style block, aspect
...
6. **Gate 5 — Vince approves the clip.** Watch one thing above all: does it still look *painted*?
...
**Two approval loops per shot** (Gate 3, Gate 5) and at least two paid generations. A shot that
...
- **Two review loops invite fatigue.** Gate 3 approvals get looser as a batch runs long. The
