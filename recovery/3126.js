const addition=`function shotFormField(label, control, hint = "") {
...
@@ -443,0 +444,${addition.split("\n").length} @@
${addition.split("\n").map(x=>"+"+x).join("\n")}`;
text(await tools.shell_command({command:`@'\n${patch}\n'@ | git apply --unidiff-zero --whitespace=nowarn -`,workdir:"C:\\Users\\vxi\\OneDrive\\Documents\\Film Study Tool",timeout_ms:10000,sandbox_permissions:"require_escalated",justification:"Allow me to add the non-technical insert/delete/restore shot controls and reversible-delete dialog to Hearthlight Studio?"}));