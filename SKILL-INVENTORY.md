...
| `hearthlight-research` | 1 | 1 | 1 | 2 | 0 | **SUPPORTING** | Feeds Stage 3 | "Research populates the world, never the story." |
...
| `hearthlight-dashboard` | 1 | 1 | 1 | 1 | 5 | **CORE** | **Primary outcome 4** | Stable Shot ID ownership plus a lightweight intake/status bridge. Studio in the Film Study Tool is the actual production surface; `scan.py` proxies its Design / Production / Inputs state rather than maintaining a second progress model. The retired stage manifest, requirements manifest, and project status ledger are gone. `rekey_assets.py` rebinds `assets.json` / `props.json` from shot number to `shot_id` under an explicit `--epoch`, refusing to guess on a collision. |
